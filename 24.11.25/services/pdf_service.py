...
======================
Fatura ve rapor PDF'leri oluşturmak için ReportLab kütüphanesini kullanan servis.

Fonksiyonlar:
- generate_invoice_pdf(order): Sipariş faturası PDF'i oluştur
- generate_report_pdf(payments): Ödeme raporu PDF'i oluştur

Özellikler:
- Türkçe karakter desteği (UTF-8)
- Tablo formatlı düzen
- Logo ekleme desteği
- Renk şeması (turuncu tema)
- Otomatik sayfalama
- Tarih formatları (İstanbul saat dilimi)

Çıktı Konumu:
- static/invoices/ - Faturalar
- static/reports/ - Raporlar
...
def register_turkish_font():
    """Türkçe karakterler için DejaVu fontlarını kaydet"""
    font_path = os.path.join('static', 'fonts', 'DejaVuSans.ttf')
    font_bold_path = os.path.join('static', 'fonts', 'DejaVuSans-Bold.ttf')
    
    if not os.path.exists(font_path):
        # Font dosyası yoksa, ReportLab'ın default fontunu kullan
        return 'Helvetica'
    
    # Normal ve Bold fontları kaydet
