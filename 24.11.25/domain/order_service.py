...
class OrderService:
    ...
    def get_payment_pending_orders(self) -> List[Order]:
        ...
        return self.order_repo.get_payment_pending_orders()
    ...
    def get_payment_pending_orders(self) -> List[Order]:
        """Ödeme bekleyen siparişleri getir"""
        return self.order_repo.get_by_status('payment_pending')
    ...
    def request_payment(self, order_id: int) -> bool:
        """Ödeme talep et"""
        order = self.order_repo.get_by_id(order_id)
        if not order or order.status not in ['pending', 'completed']:
            return False
