...
def dashboard():
    ...
    pending_orders = Order.query.filter_by(status='pending').all() #bekleyen siparişleri gösterir
    payment_pending_orders = Order.query.filter_by(status='payment_pending').all() #ödeme bekleyen siparişleri gösterir
...
def add_table():
    ...
    if form.validate_on_submit():
        # Masa adı zaten var mı kontrol et
        existing_table = Table.query.filter_by(name=form.name.data).first()
        if existing_table:
            flash('Bu masa adı zaten mevcut!', 'error')
...
def add_item_to_table(table_id):
    ...
    if form.validate_on_submit():
        # Masanın ödenmemiş siparişini bul
        current_order = (Order.query
                         .filter(Order.table_id==table_id, Order.status.in_(['pending','completed','payment_pending']))
        ...
        existing_item = OrderItem.query.filter_by(order_id=current_order.id, product_id=product.id).first()
        if existing_item:
...
def cancel_payment(order_id):
    ...
    # İşaretle: iptal edildi
    order.status = 'cancelled'

    # Eğer bu siparişe atanmış bir masa varsa ve o masada başka aktif (pending/completed/payment_pending)
    # sipariş yoksa masayı boşalt
    if order.table_id:
...
@login_required
def payments():
    """Ödeme yönetimi - Ödeme bekleyen siparişler"""
    from models import Order
    # payment_pending durumundaki siparişleri getir
    pending_payments = Order.query.filter_by(status='payment_pending').order_by(Order.updated_at.desc()).all()
...
def profile():
    ...
    if request.method == 'POST':
        ...
        # Flask-Login session'ını tamamen yenile
        # Session'daki tüm Flask-Login verilerini temizle
        session.pop('_user_id', None)
        session.pop('_fresh', None)
        session.pop('_id', None)
        
        # Kullanıcıyı tekrar giriş yap
