...
def generate_invoice_pdf(order):
    ...
    <b>Telefon:</b> {user.phone if user else 'Bilinmiyor'}<br/>
    <b>Sipariş Tarihi:</b> {order.created_at.strftime('%d.%m.%Y %H:%M')}<br/>
...
def generate_report_pdf(payments):
    ...
    # Rapor tarihi
    report_date = f"Rapor Tarihi: {datetime.now().strftime('%d.%m.%Y %H:%M')}"
