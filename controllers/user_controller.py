...
from forms import OrderItemForm, ContactForm, ReviewForm
...
def payment():
    ...
    # OrderService ile sipariş durumunu ödeme bekliyor olarak güncelle
...
def contact():
    """İletişim formu - sadece e-posta gönderimi (istek, şikayet)"""
    form = ContactForm()
    current_user = None
    
    # Eğer kullanıcı giriş yaptıysa bilgilerini doldur
    ...
    if form.validate_on_submit():
        ...
        contact_service.create_contact(
            ...
            rating=None,  # İletişim formunda puan yok
            user_id=current_user.id if current_user else None,
            is_visible=False  # İletişim formu görünür değil, sadece e-posta gönderimi
        )
...
def reviews():
    """Yorumları görüntüleme sayfası - herkes görebilir"""
    form = ReviewForm()
    current_user = None
    ...
    # Form gönderildiyse yorum oluştur
    if form.validate_on_submit():
        # Rating'i request'ten al (radio button'dan)
        ...
        contact_service.create_contact(
            name=form.name.data,
            email=form.email.data,
            contact_type='comment',  # Yorum tipi
            message=form.message.data,
            rating=rating,
            user_id=current_user.id if current_user else None,
            is_visible=True  # Yorumlar görünür
        )
